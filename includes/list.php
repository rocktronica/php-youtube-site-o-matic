<?php if ($page == 1) { 
	...
	<div class="bigvideo">
		...
		<div class="content">
			<h2><a href="?id=<?php echo $id; ?>"><?php echo stripbranding($video->title); ?></a></h2>
	...
	foreach ($youtube->feed->entry as $video) {
		if (($i !== 1) || ($page !== 1)) {
			...
			<li>
				...
				<div class="content">
					<<?php echo $h2orh3; ?>><a href="?id=<?php echo $id; ?>"><?php echo stripbranding($video->title); ?></a></<?php echo $h2orh3; ?>>
					<?php echo firstp($video->content); ?>
					<p class="published"><?php echo date("F j, Y",strtotime(substr($video->published,0,10))); ?></p>
				</div>
			</li>
